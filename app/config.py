...
WORKERS = int(os.getenv("WORKERS", "1"))  # 工作进程数 - 优化为1以降低内存占用

# OCR配置
OCR_MODEL_DIR = os.getenv("OCR_MODEL_DIR", str(BASE_DIR / "models"))
OCR_PROCESS_POOL_SIZE = int(os.getenv("OCR_PROCESS_POOL_SIZE", "2"))  # OCR处理进程池大小 - 优化为2以降低内存占用
...
ENABLE_GC_AFTER_REQUEST = os.getenv("ENABLE_GC_AFTER_REQUEST", "True").lower() in ("true", "1", "t")  # 请求后启用垃圾回收

# 日志配置
LOG_LEVEL = os.getenv("LOG_LEVEL", "WARNING")  # 优化内存使用，减少日志输出
